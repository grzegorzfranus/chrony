...
* `chrony_package_name` - The name of the Chrony package to install (defaults to "chrony").
* `chrony_service_name` - The name of the Chrony service (OS-dependent, e.g., "chronyd" for RedHat, "chrony" for Debian).
* `chrony_ntp_service_name` - The name of the NTP service to check and disable if running.
* `chrony_systemd_timesyncd_service_name` - The name of the systemd-timesyncd service to check and disable if running.
* `chrony_config_path` - The path to the Chrony configuration file.
* `chrony_log_directory_user` - The user that owns the Chrony log directory.
* `chrony_driftfile_path` - The path to the Chrony drift file.
* `chrony_keyfile_path` - The path to the Chrony keys file.
* `chrony_logrotate_config_path` - The path to the Chrony logrotate configuration file.
